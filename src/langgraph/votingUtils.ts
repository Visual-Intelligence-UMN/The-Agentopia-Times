    ...
    const voteTasks = agents.map(async (agent, i) => {
        ...
        if (index === 0) {
            const roleContent =
                ...
                `follow these statement with highest priority ${bias}` +
                agent.getBias();
        ...
        console.log(
            `[Debug] Agent ${agent.getName()} vote result: ${llmResult?.content}`,
        );
...
export function createAggregator(
    ...
    return async function aggregator(
        ...
        `); // prompt_change
            ...
            const vizVotes = votes.filter(
                (v: any) =>
                    v && typeof v === 'object' && (v.d3Code || v.vegaLite),
            );
            const llmInput = vizVotes
                .map((v: any) => v.d3Code || v.vegaLite)
                .join('\n---\n');
        ...
        const originalAgent1X = agents[agents.length - 1].x;
        const originalAgent1Y = agents[agents.length - 1].y;
...
export function constructVotingGraph(
    ...
    votingGraph.addNode('aggregator', async (state: any) => {
        ...
        if (index === 2) {
            return {
                ...state,
                votingOutput: decision.votingOutput,
