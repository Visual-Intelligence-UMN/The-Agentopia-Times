...
import {
    generateAutoVISPrompt,
    generateBiasedPrompt,
} from '../vega/visualizationLibrary';
...
The Chart should be interactive and responsive, properly titled, and labeled for each chart.
  ${
      lastError
          ? `5. ERROR FIXING: Correct these issues from last attempt:
    - ${lastError}
  ...
  `;
    ...
    // All attempts failed
    throw new Error(
        'Failed to generate valid D3.js code after 3 attempts. Last error:\n' +
            lastError,
    );
}
...
export function cleanUpD3Code(code: any) {
    ...
    code = code.replace(
        /^\s*(json|javascript|js|ts|typescript|html)\s*\n/i,
        '',
    );
...
export async function checkIfCodeCanRunInBrowser(
    code: string,
