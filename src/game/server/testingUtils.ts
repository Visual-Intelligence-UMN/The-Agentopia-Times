...
export async function testChain(
  ...
  `;
    ...
    eventTargetBus.dispatchEvent(
        new CustomEvent('signal', { detail: 'signal 1' }),
    );
    ...
    await autoControlAgent(
        ...
        agent2?.x as number,
        agent2?.y as number,
        'Send Message to ' + (agent2?.name as string),
    );
    ...
    // signal 2
    eventTargetBus.dispatchEvent(
        new CustomEvent('signal', { detail: 'signal 2' }),
    );
    await autoControlAgent(
        scene,
        agent1,
...
export async function evaluateCustomerSupportResponse(
    pattern: string,
    response: string,
) {
