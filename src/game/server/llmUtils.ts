...
export const route = async (input: string, routes: Map<string, string>) => {
    ...
    try {
        const routeResponse = (
            await callLLM([{ role: 'user', content: selectorPrompt }])
        ...
        if (!selectedPrompt) {
            throw new Error(
                `Route "${routeKey}" not found in available routes.`,
            );
        }
        const result = (
            await callLLM([
...
async function judgeDebate(messages: any) {
    const judgeResponse = await callLLM([
        ...messages,
        {
            role: 'user',
            content:
                'Judge, analyze the arguments and declare which agent made stronger points. Keep it brief.',
        },
    ]);

    const verdict = judgeResponse.choices[0].message.content;
...
export async function debate(topic: string, rounds: number) {
    ...
    const messages = [
        {
            role: 'system',
            content: `You are participating in a structured debate. 
          Agent A will support the topic: "${topic}", while Agent B will oppose it.
          Each response should be concise (maximum 2 sentences).`,
        },
        {
            role: 'user',
            content: `Agent A, start the debate by stating your main argument for: "${topic}".`,
        },
    ];
    ...
    for (let i = 0; i < rounds; i++) {
        ...
        const responseA = await callLLM(messages);
        const textA = responseA.choices[0].message.content;
