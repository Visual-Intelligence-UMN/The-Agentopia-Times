...
export class Agent extends Phaser.Physics.Arcade.Sprite {
    ...
    public addMssgSprite(
        ...
        this.mssgSprite = scene.add
            .image(this.x, this.y, texture, frame)
            .setOrigin(0.5, 1)
            .setDepth(10);
    ...
    private static rebalanceBiasTypes(scene: Phaser.Scene) {
        const pool = (scene.registry.get('biasTypePool') as string[]) || [
            'factual',
        ];
    ...
    private createWorkAnimations(atlasKey: string) {
        ...
        const animList = [{ key: 'player_work', prefix: 'misa-work.' }];
    ...
    public setAgentState(state: 'work' | 'idle') {
        if (state === 'work') {
            ...
            this.createAnimations(
                this.isBiased ? key.atlas.bias : key.atlas.player,
            );
        }
    }
