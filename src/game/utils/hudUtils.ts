...
export function addAgentPanelHUD(
    ...
    this.hudText = this.add.text(50, 450, `Agent (Player-controlled) `, {
        fontSize: '18px',
        color: '#ffffff',
        ...
        btn.on('pointerdown', () => {
            ...
            if (i != 0) {
                ...
                const agent = new Agent(
                    ...
                    'player',
                    'misa-front',
                    'Agent ' + this.controllableCharacters.length,
                );
...
function createTaskCard(this: any, x: number, y: number, roomName: string) {
    ...
    task.on('dragend', () => {
        ...
        this.tweens.add({
            ...
            x: (target: any) =>
                target === task ? originalPos.x : originalPos.labelX,
            y: (target: any) =>
                target === task ? originalPos.y : originalPos.labelY,
            duration: 300,
            ease: 'Back.easeOut',
            onComplete: () => {
...
export function addTaskAssignmentHUD(this: any) {
    const rooms = ['analysis', 'summary', 'extraction', 'validation', 'voting'];
    for (let i = 0; i < rooms.length; i++) {
        const x = 425 + i * 60;
        const y = 540;
...
export function generateNonCollidingAgentPosition(
    existingAgents: any,
    bounds: any,
    agentRadius = 20,
    margin = 10,
    ...
    do {
        agentX =
            bounds.topLeft.x +
            margin +
            Math.random() * (bounds.width - 2 * margin);
        agentY =
            bounds.topLeft.y +
            margin +
            Math.random() * (bounds.height - 2 * margin);
    ...
    if (attempts >= maxAttempts) {
        console.warn(
            'Failed to place agent without collision after max attempts.',
        );
    }
...
export const getZoneBounds = (zone: any) => {
    return {
        topLeft: {
            x: zone.x - zone._displayOriginX,
            y: zone.y - zone._displayOriginY,
        },
        bottomRight: {
            x: zone.x - zone._displayOriginX + zone.width,
            y: zone.y - zone._displayOriginY + zone.height,
        },
        width: zone.width,
        height: zone.height,
    };
