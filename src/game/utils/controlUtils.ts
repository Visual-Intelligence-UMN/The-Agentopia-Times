...
const pathGraphics: Phaser.GameObjects.Graphics | null = null;
const targetCircle: Phaser.GameObjects.Graphics | null = null;
...
export function isClickOnHUD(
    pointer: Phaser.Input.Pointer,
    hudElements: Phaser.GameObjects.GameObject[],
): boolean {
    return hudElements.some((hud) => {
        if (
            hud instanceof Phaser.GameObjects.Rectangle ||
            hud instanceof Phaser.GameObjects.Text
        ) {
...
export async function transmitReport(
    ...
    return new Promise(async (resolve) => {
        ...
        await popupEvent(
            ...
            report.x,
            report.y,
            'Send Report to Next Department',
        );
    });
}
...
function createTargetCircle(
    ...
    agentWithGraphics.targetCircle.strokeCircle(
        targetX,
        targetY,
        tilemap.tileWidth / 2,
    );
...
export function controlCameraMovements(
    scene: Phaser.Scene,
    cursors: any,
    speed: number = 5,
) {
