...
export class TextInput {
    ...
    constructor(
        ...
        this.inputText = scene.add
            .text(x - width / 2 + padding, y - 10, this.text, {
                fontSize: '24px',
                color: textColor,
                wordWrap: {
        ...
        scene.input.keyboard!.on('keydown', (event: KeyboardEvent) => {
            ...
            if (this.inputText.width > this.inputWidth - 2 * padding) {
                this.inputText.x =
                    x -
                    this.inputWidth / 2 -
                    (this.inputText.width - (this.inputWidth - 2 * padding));
                ...
                try {
                    ...
                    if (this.inputText.width > this.inputWidth - 2 * padding) {
                        this.inputText.x =
                            x -
                            this.inputWidth / 2 -
                            (this.inputText.width -
                                (this.inputWidth - 2 * padding));
                        this.cursorText.x =
                            this.inputText.x + this.inputText.width - 5;
                    } else {
                        this.inputText.x = x - this.inputWidth / 2 + padding;
                        this.cursorText.x =
                            this.inputText.x + this.inputText.width - 5;
        ...
        scene.input.on('pointerdown', (pointer: Phaser.Input.Pointer) => {
            if (
                ...
                this.inputBackground.getBounds().contains(pointer.x, pointer.y)
            ) {
    ...
    private showPasteButton(scene: Phaser.Scene, x: number, y: number) {
        ...
        this.pasteButton = scene.add
            .text(x + 10, y + 10, 'Paste', {
                fontSize: '20px',
                color: '#000000',
        ...
        this.pasteButton.on('pointerdown', async () => {
            try {
                ...
                if (this.inputText.width > this.inputWidth - 2 * 10) {
                    this.inputText.x =
                        this.inputBackground.x -
                        this.inputWidth / 2 -
                        (this.inputText.width - (this.inputWidth - 2 * 10));
                    this.cursorText.x =
                        this.inputText.x + this.inputText.width - 5;
                } else {
                    this.inputText.x =
                        this.inputBackground.x - this.inputWidth / 2 + 10;
                    this.cursorText.x =
                        this.inputText.x + this.inputText.width - 5;
                }
            } catch (error) {
